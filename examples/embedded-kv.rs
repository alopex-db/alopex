...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let txn = db.begin(TxnMode::ReadOnly)?;
    println!("Reading 'key'...");
    ...
    // No need to call rollback explicitly here.
    
    println!("\nDropping DB and re-opening to test persistence...");
    drop(db);

    let db2 = Database::open(&path)?;
    let txn2 = db2.begin(TxnMode::ReadOnly)?;
    let val2 = txn2.get(b"key")?;
    println!("Read 'key' after re-opening: {}", String::from_utf8_lossy(&val2.unwrap()));
