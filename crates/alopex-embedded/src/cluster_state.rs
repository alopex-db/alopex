...
impl EmbeddedClusterState {
    ...
    /// Adds one member through the shared durable Set projection and returns
    /// the canonical local-only outcome for the standalone embedded database.
    pub(crate) fn add_set(
        &mut self,
        store: &AnyKV,
        envelope: CrdtOperationEnvelope,
    ) -> CrdtOutcome {
        if envelope.operation != CrdtOperationKind::SetAdd {
            return self.set_rejection(
                &envelope,
                FailureClass::InvalidRequest,
                "set_add_envelope_required",
            );
        }
