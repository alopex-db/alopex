...
impl PyDatabase {
    ...
    /// Create an empty Set through the shared local CRDT projection.
    #[pyo3(
    ...
    fn create_set(
        ...
        let envelope = alopex_cluster::CrdtOperationEnvelope::new(
            ...
            operation_id,
            update_version,
            alopex_cluster::CrdtOperationKind::SetCreate,
            alopex_cluster::CrdtPayload::None,
        )
        .map_err(|error| error::to_py_err(error.to_string()))?;
        let outcome = py
            .detach(move || db.create_set(envelope))
            .map_err(error::embedded_err)?;
        let status = crdt_outcome_to_py(py, &outcome)?;
        if let Some(code) = outcome.surface_status().python_error_code {
            let py_err = error::with_code(
    ...
    /// Add one member to an existing Set through the shared local CRDT
    /// projection.
    #[pyo3(
        signature = (object_id, *, cluster_id, table_id, range_id, schema_version, data_epoch, request_id, operation_id, update_version, member, actor = "alopex-python-local")
    )]
    #[allow(clippy::too_many_arguments)]
    fn add_set(
        &self,
        ...
        let envelope = alopex_cluster::CrdtOperationEnvelope::new(
            ...
            operation_id,
            update_version,
            alopex_cluster::CrdtOperationKind::SetAdd,
            alopex_cluster::CrdtPayload::Set {
                member: Some(member.to_owned()),
            },
        )
        .map_err(|error| error::to_py_err(error.to_string()))?;
        let outcome = py
            .detach(move || db.add_set(envelope))
            .map_err(error::embedded_err)?;
        let status = crdt_outcome_to_py(py, &outcome)?;
        if let Some(code) = outcome.surface_status().python_error_code {
            let py_err = error::with_code(
    ...
    /// Remove one member from an existing Set through the shared local CRDT
    /// projection.
    #[pyo3(
        signature = (object_id, *, cluster_id, table_id, range_id, schema_version, data_epoch, request_id, operation_id, update_version, member, actor = "alopex-python-local")
    )]
    ...
    fn remove_set(
        ...
        let envelope = alopex_cluster::CrdtOperationEnvelope::new(
            ...
            operation_id,
            update_version,
            alopex_cluster::CrdtOperationKind::SetRemove,
            alopex_cluster::CrdtPayload::Set {
                member: Some(member.to_owned()),
            },
        )
        .map_err(|error| error::to_py_err(error.to_string()))?;
        let outcome = py
            .detach(move || db.remove_set(envelope))
            .map_err(error::embedded_err)?;
        let status = crdt_outcome_to_py(py, &outcome)?;
        if let Some(code) = outcome.surface_status().python_error_code {
            let py_err = error::with_code(
    ...
    /// Check one member against an existing Set without changing its durable
    /// projection or recording an operation in the Set ledger.
    #[pyo3(
        signature = (object_id, *, cluster_id, table_id, range_id, schema_version, data_epoch, request_id, operation_id, update_version, member, actor = "alopex-python-local")
    )]
    #[allow(clippy::too_many_arguments)]
    fn contains_set(
        ...
        let envelope = alopex_cluster::CrdtOperationEnvelope::new(
            ...
            operation_id,
            update_version,
            alopex_cluster::CrdtOperationKind::SetContains,
            alopex_cluster::CrdtPayload::Set {
                member: Some(member.to_owned()),
            },
        )
        .map_err(|error| error::to_py_err(error.to_string()))?;
        let outcome = py
            .detach(move || db.contains_set(envelope))
            .map_err(error::embedded_err)?;
        let status = crdt_outcome_to_py(py, &outcome)?;
        if let Some(code) = outcome.surface_status().python_error_code {
            let py_err = error::with_code(
    ...
    /// List the canonical members of an existing Set without changing its
    /// durable projection or recording an operation in the Set ledger.
    #[pyo3(
        signature = (object_id, *, cluster_id, table_id, range_id, schema_version, data_epoch, request_id, operation_id, update_version, actor = "alopex-python-local")
    )]
    #[allow(clippy::too_many_arguments)]
    fn list_set(
        ...
        let envelope = alopex_cluster::CrdtOperationEnvelope::new(
            ...
            operation_id,
            update_version,
            alopex_cluster::CrdtOperationKind::SetList,
            alopex_cluster::CrdtPayload::None,
        )
        .map_err(|error| error::to_py_err(error.to_string()))?;
        let outcome = py
            .detach(move || db.list_set(envelope))
            .map_err(error::embedded_err)?;
        let status = crdt_outcome_to_py(py, &outcome)?;
        if let Some(code) = outcome.surface_status().python_error_code {
            let py_err = error::with_code(
    ...
    /// Apply a signed delta to subtract from an existing Counter through the
    /// shared local CRDT projection.
    #[pyo3(
        signature = (object_id, *, cluster_id, table_id, range_id, schema_version, data_epoch, request_id, operation_id, update_version, delta, actor = "alopex-python-local")
    )]
    #[allow(clippy::too_many_arguments)]
    fn decrement_counter(
        ...
        let envelope = alopex_cluster::CrdtOperationEnvelope::new(
            ...
            operation_id,
            update_version,
            alopex_cluster::CrdtOperationKind::CounterDecrement,
            alopex_cluster::CrdtPayload::Counter {
                initial_value: None,
                delta: Some(delta),
            },
        ...
        .map_err(|error| error::to_py_err(error.to_string()))?;
        let outcome = py
            .detach(move || db.decrement_counter(envelope))
            .map_err(error::embedded_err)?;
        let status = crdt_outcome_to_py(py, &outcome)?;
        if let Some(code) = outcome.surface_status().python_error_code {
            let py_err = error::with_code(
    ...
    /// Read a Set through the shared local CRDT projection without recording
    /// a mutation in its durable operation ledger.
    #[pyo3(
        signature = (object_id, *, cluster_id, table_id, range_id, schema_version, data_epoch, request_id, operation_id, update_version, actor = "alopex-python-local")
    )]
    #[allow(clippy::too_many_arguments)]
    fn read_set(
        ...
        let envelope = alopex_cluster::CrdtOperationEnvelope::new(
            ...
            operation_id,
            update_version,
            alopex_cluster::CrdtOperationKind::SetRead,
            alopex_cluster::CrdtPayload::None,
        )
        .map_err(|error| error::to_py_err(error.to_string()))?;
        let outcome = py
            .detach(move || db.read_set(envelope))
            .map_err(error::embedded_err)?;
        let status = crdt_outcome_to_py(py, &outcome)?;
        if let Some(code) = outcome.surface_status().python_error_code {
            let py_err = error::with_code(
