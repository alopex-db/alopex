...
class AsyncDatabase(_AsyncLocalHandle):
    ...
    async def create_counter(
        ...
        *,
        ...
        """Create a local CRDT Counter with the canonical Phase 2 outcome mapping.

        The owned native database performs the operation after releasing the
        GIL; this facade intentionally preserves its canonical result and
        ``AlopexError`` status mapping without starting a Python worker or a
        remote client.
        """
        return self._handle.create_counter(
            object_id,
    ...
    async def create_set(
        ...
        *,
        ...
        """Create a local CRDT Set with the canonical Phase 2 outcome mapping.

        This facade delegates to the existing native binding and neither starts
        a Python worker nor creates a remote client.
        """
        return self._handle.create_set(
            object_id,
            cluster_id=cluster_id,
            table_id=table_id,
            range_id=range_id,
    ...
    async def add_set(
        ...
        *,
        ...
        """Add a member to a local CRDT Set with the canonical Phase 2 outcome.
    ...
    async def remove_set(
        ...
        *,
        ...
        """Remove a member from a local CRDT Set with the canonical Phase 2 outcome.

        This facade forwards directly to the GIL-releasing native binding. It
        does not create a Python worker, remote client, or Python-side Set
        projection.
        """
        return self._handle.remove_set(
            object_id,
            cluster_id=cluster_id,
            table_id=table_id,
            range_id=range_id,
    ...
    async def contains_set(
        ...
        *,
        ...
        """Check one Set member without mutating its durable operation ledger.

        This facade forwards directly to the GIL-releasing native binding. It
        does not create a Python worker, remote client, or Python-side Set
        projection.
        """
        return self._handle.contains_set(
            object_id,
            cluster_id=cluster_id,
            table_id=table_id,
            range_id=range_id,
