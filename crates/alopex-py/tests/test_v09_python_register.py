...
I22_ROWS: tuple[tuple[str, str, str], ...] = (
    *(("PyDatabase", method, DATABASE_SOURCE) for method in (
        ...
        "memory_usage", "cluster_status", "routing_diagnostics", "create_counter", "create_set", "read_counter", "increment_counter", "decrement_counter", "close",
...
def test_i22_counter_bindings_have_public_python_stubs() -> None:
    for method in ("create_counter", "create_set", "read_counter", "increment_counter", "decrement_counter"):
        assert re.search(
            rf"^    def {method}\s*\(",
            STUB_SOURCE,
            re.MULTILINE,
