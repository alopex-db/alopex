...
I22_ROWS: tuple[tuple[str, str, str], ...] = (
    *(("PyDatabase", method, DATABASE_SOURCE) for method in (
        ...
        "memory_usage", "cluster_status", "routing_diagnostics", "create_counter", "create_set", "add_set", "remove_set", "contains_set", "list_set", "read_counter", "read_set", "increment_counter", "decrement_counter", "close",
...
def test_i22_counter_bindings_have_public_python_stubs() -> None:
    for method in ("create_counter", "create_set", "add_set", "remove_set", "contains_set", "list_set", "read_counter", "read_set", "increment_counter", "decrement_counter"):
        assert re.search(
            rf"^    def {method}\s*\(",
            STUB_SOURCE,
            re.MULTILINE,
...
def test_i22_set_read_has_async_implementation_and_stub() -> None:
    for source in (ASYNCIO_SOURCE, ASYNCIO_STUB_SOURCE):
        assert re.search(
...
def test_i22_set_remove_has_async_implementation_and_stub() -> None:
    for source in (ASYNCIO_SOURCE, ASYNCIO_STUB_SOURCE):
        assert re.search(
            r"^    async def remove_set\s*\(", source, re.MULTILINE
        ), "missing public AsyncDatabase.remove_set"


def test_i22_set_contains_has_async_implementation_and_stub() -> None:
    for source in (ASYNCIO_SOURCE, ASYNCIO_STUB_SOURCE):
        assert re.search(
            r"^    async def contains_set\s*\(", source, re.MULTILINE
        ), "missing public AsyncDatabase.contains_set"


def test_i22_set_list_has_async_implementation_and_stub() -> None:
    for source in (ASYNCIO_SOURCE, ASYNCIO_STUB_SOURCE):
        assert re.search(
