...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            lambda np, base: np.array(base, dtype=np.float32).reshape((-1, 1))[:, 0],
            id="f32_strided_view",
        ),
    ],
)
def test_search_similar_accepts_dtype_and_layout_variations(query_factory):
    ...
    with db.begin(TxnMode.READ_WRITE) as txn:
        ...
        results = txn.search_similar(query, Metric.COSINE, 1)

    assert len(results) == 1
    assert results[0].key == b"a"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            lambda np: np.array([4.0, 5.0, 6.0], dtype=np.float32).reshape((-1, 1))[:, 0],
            id="f32_strided_view",
        ),
    ],
)
