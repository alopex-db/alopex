...
fn index_predicate(
    ...
    let crate::planner::typed_expr::TypedExprKind::BinaryOp { left, op, right } = &predicate.kind
    else {
        return None;
    };
    ...
    let (column, value, reversed) = match (&left.kind, &right.kind) {
        (
            crate::planner::typed_expr::TypedExprKind::ColumnRef { column_index, .. },
            crate::planner::typed_expr::TypedExprKind::Literal(_)
            | crate::planner::typed_expr::TypedExprKind::VectorLiteral(_),
        ) => (
            *column_index,
...
fn execute_index_scan<'txn, S: KVStore + 'txn>(
    ...
    Ok(txn.with_table(table_meta, |storage| {
        row_ids
            .into_iter()
            .filter_map(|row_id| match storage.get(row_id) {
                Ok(Some(values)) => Some(Ok(Row::new(row_id, values))),
                Ok(None) => None,
