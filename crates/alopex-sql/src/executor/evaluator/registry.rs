...
use super::{
    EvalContext, conditional, datetime, fts, hash, json, nested, numeric, string, type_fn,
};
...
fn evaluator_for(name: &str) -> (EvalFn, Option<LazyEvalFn>) {
    match name {
        "memory_stats" | "io_stats" | "clear_cache" | "nextval" => (system_placeholder, None),
        ...
        _ => {
            if let Some(eval) = datetime::eval_for(name) {
                ...
                (eval, None)
            } else if let Some(eval) = fts::eval_for(name) {
                (eval, None)
            } else if let Some(eval) = json::eval_for(name) {
                (eval, None)
            } else if let Some(eval) = nested::eval_for(name) {
                (eval, None)
            } else {
                (unsupported, None)
            }
