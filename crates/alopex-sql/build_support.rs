...
pub(crate) const LEGACY_ALOPEX_VERSION: &str = "0.8.4";
pub(crate) const LEGACY_CONTRACT_VERSION: &str = "0.4.0";
...
fn verify_contract_sidecar(directory: &Path) -> Result<(), ResolveError> {
    ...
    let accepted_contracts = [
        ...
        format!("{LEGACY_CONTRACT_VERSION}\n"),
    ];
    if !accepted_contracts.iter().any(|expected| contract == expected.as_bytes()) {
        return Err(error(format!(
...
fn validate_manifest(manifest: &VendorManifest) -> Result<(), ResolveError> {
    if manifest.schema != VENDOR_MANIFEST_SCHEMA && manifest.schema != LEGACY_VENDOR_MANIFEST_SCHEMA {
        return Err(error("parser vendor manifest schema mismatch"));
    }
    if manifest.schema == VENDOR_MANIFEST_SCHEMA {
        let accepted_alopex_versions = [REQUIRED_ALOPEX_VERSION, LEGACY_ALOPEX_VERSION];
        if !accepted_alopex_versions.contains(&manifest.alopex_version.as_str()) {
            return Err(error("parser vendor manifest Alopex version mismatch"));
        }
        let accepted_contract_versions = [REQUIRED_CONTRACT_VERSION, LEGACY_CONTRACT_VERSION];
        if !accepted_contract_versions.contains(&manifest.contract_version.as_str()) {
