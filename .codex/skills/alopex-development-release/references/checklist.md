...
- [ ] If publication fails or is cancelled before any public artifact exists, cancel active workflows, delete every unpublished release tag immediately, record the rollback, then return to the corrected-main tagging step.
- [ ] When changing this recovery policy, update every source in the [release recovery policy source map](../../../../docs/release-pipeline-responsibilities.md#release-recovery-policy-source-map) and run `scripts/release/tests/test_release_contract.py`.
