...
- Current contract version: `0.15.0`, returned by `alopex_parser_version()`.
...
- A non-zero parse error is returned as `prkError`; no Nim exception crosses
  the C ABI boundary.
- Contract `0.15.0` is compatibility metadata inside the Alopex release; it is
...
report exactly `0.15.0`. A mismatch is rejected before MessagePack decoding;
...
describes the current `0.15.0` surface and does not retroactively change them.
