...
current `0.21.0` requirement. Alopex version, source-tree digest, archive digest,
...
that bytes relabeled as `0.21.0` export that contract. Rust therefore compares
`alopex_parser_version()` with the compiled `0.21.0` descriptor before payload
