...
current `0.8.0` requirement. Alopex version, source-tree digest, archive digest,
...
that bytes relabeled as `0.8.0` export that contract. Rust therefore compares
`alopex_parser_version()` with the compiled `0.8.0` descriptor before payload
...
| four vendor target directories | immutable historical v0.8.4 / 0.4.0 and v0.8.7 / 0.5.0 assets | regenerated v0.8.8 / contract 0.8.0 binaries | replace only from release staging | only after all target attestations pass; never relabel old bytes | manifest/archive/library SHA checks |
