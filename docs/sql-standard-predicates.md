...
row-value comparisons on every SQL execution surface. Parser contract `0.10.0`
...
assets using current contract `0.10.0`, retarget the manifest and wheels, and
...
| --- | --- | --- | --- | --- | --- |
| `PARSER_CONTRACT_VERSION` and Rust consumer pin | select the current wire schema | require contract `0.10.0` | keep in sync | every schema-changing parser commit | contract mismatch and exported-version tests |
...
| four target parser archives and manifest | release-native parser identity | carry fresh contract-0.10.0 bytes | replace in release staging | all target records and native smoke checks pass | release join verifier |
