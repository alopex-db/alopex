...
| --- | --- | --- | --- | --- | --- |
| Nim parser and Rust AST | `DATE`/`TIME` collapsed into `TIMESTAMP` | emit distinct types and typed casts | replace the collapsing mapping; bump parser contract to `0.19.0` | old mapping disappears when new parser assets are staged | parser and exported-contract tests |
...
| release verifier | parser contract and prior type gate | require temporal demo and all public surfaces | stage fresh `0.19.0` parser assets | stale `0.15.0` assets fail closed | release tests and demo |
...
- Parser tests cover DDL types, typed literals, casts, and contract `0.19.0`.
